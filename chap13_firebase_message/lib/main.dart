...
- 다양한 도구와 서비스를 제공하여 개발 시간을 단축하고, 효율성을 높을 수 있다.

- db, 인증, 서버가 없을 시 백엔드 코드 실행 등도 제공한다.
  하지만, 우리는 spring과 mysql을 사용하므로 우리는 알리만 사용해 볼 것이다.
- 클라우드 저장소도 1GB 무료로 제공한다.

Firebase Cloud Messaging (FCM)
- 무료 푸시 알림 서비스. 사용자에게 실기간 메시지를 보낼 수 있다.
* */
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> sendTokenToServer(String token) async{
    final reponse = await http.post(
      Uri.parse("http://localhost:8080"), // 서버 url
